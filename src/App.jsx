...
import {
  haversineMeters, formatDistance, monthLabel, localDateStr, fmtAbsenTime,
  fmtWorkDuration, excelDateSerial, excelDateTimeSerial, setColDateFormat,
} from './lib/format';
import {
...
function VisitDetailModal({ visit, bengkel, kota, distributor, md, onClose, onDeleted, canEdit = false, onUpdated, bengkels = [], distributors = [], canCheck = false, checkerId = null, canEditRemarks = false, editRequest = null, onRequestEdit = null }) {
  ...
  const startEdit = () => {
    setForm({
      ...
      status: visit.status || HASIL_TERPASANG,
...
function MDDashboard({ currentMD, visits, bengkels, kotas }) {
  ...
  const pemasangan = monthVisits.filter(v => v.status === HASIL_TERPASANG).length;
  const revisit = monthVisits.filter(v => v.status !== HASIL_TERPASANG).length;
...
function VisitHistory({ visits, bengkels, kotas, distributors, currentMD, onUpdated, reqByVisit = new Map(), onReqChanged }) {
  ...
  useEffect(() => { setVisibleCount(50); }, [search, month, status, dari, sampai]);
  ...
  const filtered = visits.filter(v => {
    ...
    if (search.trim()) {
      ...
      const hay = `${b?.code || ''} ${b?.name || v.bengkel_name || ''} ${v.status || ''}`.toLowerCase();
...
function LaporanTab({ visits, bengkels, kotas, regions, accounts = [], distributors = [], refCounts = [], mode = 'wilayah' }) {
  ...
  const { groups, grand } = useMemo(() => {
    const blank = () => ({
      ...
      seen: new Set(),                                   // bengkel target yang sudah divisit
      cats: Object.fromEntries(CATS.map(c => [c, 0])),
    });
    ...
    const grand = blank();
    groups.forEach(g => {
      SUM_KEYS.forEach(k => { grand[k] += g.sub[k]; });
  ...
  // Isi 1 sel: angka polos (raw) atau "n (x%)" terhadap penyebut kolom itu.
  const fmt = (c, r) => (c.raw ? c.get(r) : cell(c.get(r), c.den ? c.den(r) : r.target));
  ...
  const filtered = useMemo(() => {
    return visits.filter(v => {
      ...
      if (filters.search.trim()) {
        ...
        const haystack = `${b?.code || ''} ${b?.name || v.bengkel_name || ''} ${md?.full_name || v.md_name || ''} ${v.status || ''}`.toLowerCase();
...
function MDForm({ regions, onSave, initial, onCancel }) {
  ...
  return (
    <div className={`mb-5 p-4 bg-slate-950 border rounded-xl ${isEdit ? 'border-amber-600/40' : 'border-slate-800'}`}>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <Field label="Email" required>
          <Input type="email" placeholder="nama@federal.id" value={form.email}
            disabled={isEdit}
